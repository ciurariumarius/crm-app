...
import { ArrowDownUp, Check, ChevronDown, ChevronLeft, ChevronRight, Loader2, MoreHorizontal, Pencil, RotateCcw, SlidersHorizontal, X } from "lucide-react"
...
import { setProjectPaymentMethod, setProjectPaymentState } from "@/lib/actions/projects"
...
export function PaymentBalancesTable({ rows, projects, partners, paymentMethods, filters, pagination }: PaymentBalancesTableProps) {
    ...
    const togglePaidState = async (row: PaymentBalanceRow, restore = false) => {
        ...
        else toast.success("Payment reverted to unpaid", { duration: 8000, action: { label: "Undo", onClick: () => void togglePaidState(row, true) } })
        router.refresh()
    }
    ...
    const controls = <PaymentLogFilterFields filters={filters} projects={projects} partners={partners} buildHref={buildHref} onNavigate={(href) => { router.push(href); setMobileFiltersOpen(false) }} />
    const clearHref = buildHref({ projectId: "all", partnerId: "all", type: "All", paidFrom: null, paidTo: null, balanceSort: "paid_recent" })
            ...
            <div className="hidden overflow-x-auto rounded-[20px] border border-[var(--line-subtle)] bg-[var(--surface-lowest)] shadow-[var(--shadow-apple)] md:block">
                <table className="w-full min-w-[1020px] table-fixed">
                    <thead className="border-b border-[var(--line-subtle)] bg-[var(--surface-low)]/70 text-left"><tr>
                        ...
                        <th className="w-[160px] px-4 py-3 text-xs font-bold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Method</th>
                    ...
                    <tbody>{visibleRows.map((row) => { const loading = pendingId === row.id; const method = methodOverrides[row.id] || row.paymentMethod; return <tr key={row.id} className="border-b border-[var(--line-subtle)] last:border-0 hover:bg-[var(--surface-low)]/50"><td className="px-4 py-3"><Link href={`/projects?status=All&projectId=${row.id}`} className="block min-w-0"><p className="truncate text-sm font-bold text-[var(--text-primary)]">{row.label}</p><p className="mt-0.5 truncate text-xs font-medium text-[var(--text-muted)]">{row.partnerName}</p></Link></td><td className="px-4 py-3"><p className="text-xs font-semibold text-[var(--text-secondary)]">{row.isRecurring ? "Recurring" : "One-time"}</p><p className="mt-0.5 truncate text-xs text-[var(--text-muted)]">{row.serviceLabel}</p></td><td className="px-4 py-3 text-right font-mono text-sm font-bold text-[var(--text-primary)]">{formatCurrency(row.currentFee)}</td><td className="px-4 py-3"><PaymentMethodMenu row={row} value={method} methods={availableMethods} disabled={loading} onChange={(value) => updateMethod(row, value)} /></td><td className="px-4 py-3 text-right text-xs font-semibold text-[var(--text-secondary)]">{row.paidAt ? format(new Date(row.paidAt), "dd MMM yyyy") : "Date unavailable"}</td><td className="px-4 py-3 text-right"><PaymentActionsMenu row={row} loading={loading} onRevert={() => void togglePaidState(row)} /></td></tr> })}</tbody>
                </table>
            </div>

            <div className="grid gap-3 md:hidden">{visibleRows.map((row) => { const loading = pendingId === row.id; const method = methodOverrides[row.id] || row.paymentMethod; return <article key={row.id} className="rounded-[18px] border border-[var(--line-subtle)] bg-[var(--surface-lowest)] p-4 shadow-[var(--shadow-apple)]"><div className="flex items-start justify-between gap-3"><Link href={`/projects?status=All&projectId=${row.id}`} className="min-w-0"><h3 className="line-clamp-2 text-[15px] font-bold text-[var(--text-primary)]">{row.label}</h3><p className="mt-1 text-xs font-medium text-[var(--text-muted)]">{row.partnerName} · {row.isRecurring ? "Recurring" : "One-time"}</p></Link><PaymentActionsMenu row={row} loading={loading} onRevert={() => void togglePaidState(row)} /></div><div className="mt-4 grid grid-cols-2 gap-3 border-t border-[var(--line-subtle)] pt-3"><div><p className="text-xs text-[var(--text-muted)]">Amount</p><p className="mt-1 font-mono text-sm font-bold">{formatCurrency(row.currentFee)}</p></div><div className="text-right"><p className="text-xs text-[var(--text-muted)]">Paid on</p><p className="mt-1 text-xs font-semibold">{row.paidAt ? format(new Date(row.paidAt), "dd MMM yyyy") : "Date unavailable"}</p></div><div className="col-span-2"><p className="mb-1 text-xs text-[var(--text-muted)]">Method</p><PaymentMethodMenu row={row} value={method} methods={availableMethods} disabled={loading} onChange={(value) => updateMethod(row, value)} /></div></div></article> })}</div>
...
function PaymentActionsMenu({ row, loading, onRevert }: { row: PaymentBalanceRow; loading: boolean; onRevert: () => void }) {
    return <DropdownMenu><DropdownMenuTrigger asChild><button type="button" disabled={loading} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-[var(--text-muted)] hover:bg-[var(--surface-low)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-primary)]" aria-label={`Payment actions for ${row.label}`}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoreHorizontal className="h-4 w-4" />}</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-xl p-1.5"><DropdownMenuItem onSelect={onRevert} className="min-h-10 cursor-pointer rounded-lg px-3 font-semibold text-[var(--state-urgent)] focus:text-[var(--state-urgent)]"><RotateCcw className="mr-2 h-4 w-4" />Revert to unpaid</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
}
...
function PaymentMethodMenu({ row, value, methods, disabled, onChange }: { row: PaymentBalanceRow; value: string | null; methods: string[]; disabled: boolean; onChange: (value: string) => Promise<boolean> }) {
    ...
    return <><DropdownMenu><DropdownMenuTrigger asChild><button type="button" disabled={disabled} className="inline-flex min-h-8 max-w-full items-center gap-1.5 rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-lowest)] px-2.5 text-xs font-semibold text-[var(--text-secondary)] hover:bg-[var(--surface-low)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-primary)]" aria-label={`Payment method for ${row.label}`}><span className="truncate">{value || "Add method"}</span><ChevronDown className="h-3.5 w-3.5 shrink-0" /></button></DropdownMenuTrigger><DropdownMenuContent align="start" className="w-48 rounded-xl p-1.5">{methods.map((method) => <DropdownMenuItem key={method} onSelect={() => void onChange(method)} className="cursor-pointer rounded-lg"><Check className={cn("mr-2 h-4 w-4", value === method ? "opacity-100" : "opacity-0")} />{method}</DropdownMenuItem>)}<DropdownMenuSeparator /><DropdownMenuItem onSelect={() => setCustomOpen(true)} className="cursor-pointer rounded-lg"><Pencil className="mr-2 h-4 w-4" />Add another…</DropdownMenuItem></DropdownMenuContent></DropdownMenu><Dialog open={customOpen} onOpenChange={setCustomOpen}><DialogContent className="sm:max-w-sm"><DialogHeader><DialogTitle>Add payment method</DialogTitle></DialogHeader><Input value={customValue} onChange={(event) => setCustomValue(event.target.value)} maxLength={64} placeholder="Method name" autoFocus onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void saveCustom() } }} /><DialogFooter><Button type="button" variant="outline" onClick={() => setCustomOpen(false)}>Cancel</Button><Button type="button" onClick={() => void saveCustom()} disabled={!normalizePaymentMethod(customValue) || savingCustom}>{savingCustom ? <Loader2 className="h-4 w-4 animate-spin" /> : null}Save</Button></DialogFooter></DialogContent></Dialog></>
}
...
function PaymentLogFilterFields({ filters, projects, partners, buildHref, onNavigate }: { filters: PaymentLogFilters; projects: { id: string; name: string }[]; partners: { id: string; name: string }[]; buildHref: (overrides: Record<string, string | null>) => string; onNavigate: (href: string) => void }) { const [draft, setDraft] = React.useState(filters); const set = (key: keyof PaymentLogFilters, value: string) => setDraft((current) => ({ ...current, [key]: value })); return <div className="space-y-4"><FilterSelect value={draft.projectId} onValueChange={(value) => set("projectId", value)} options={[{ label: "All projects", value: "all" }, ...projects.map((project) => ({ label: project.name, value: project.id }))]} /><FilterSelect value={draft.partnerId} onValueChange={(value) => set("partnerId", value)} options={[{ label: "All partners", value: "all" }, ...partners.map((partner) => ({ label: partner.name, value: partner.id }))]} /><FilterSelect value={draft.type} onValueChange={(value) => set("type", value)} options={[{ label: "All types", value: "All" }, { label: "Recurring", value: "Recurring" }, { label: "One-time", value: "OneTime" }]} /><FilterSelect value={draft.sort} onValueChange={(value) => set("sort", value)} options={[{ label: "Recently paid", value: "paid_recent" }, { label: "Amount high to low", value: "amount_desc" }, { label: "Amount low to high", value: "amount_asc" }]} /><div className="grid grid-cols-2 gap-3"><Input type="date" value={draft.paidFrom} onChange={(event) => set("paidFrom", event.target.value)} aria-label="Paid from" /><Input type="date" value={draft.paidTo} onChange={(event) => set("paidTo", event.target.value)} aria-label="Paid to" /></div><div className="flex justify-end gap-2 pt-2"><Button type="button" variant="ghost" onClick={() => setDraft({ projectId: "all", partnerId: "all", type: "All", sort: "paid_recent", paidFrom: "", paidTo: "" })}>Reset</Button><Button type="button" onClick={() => onNavigate(buildHref({ projectId: draft.projectId, partnerId: draft.partnerId, type: draft.type, balanceSort: draft.sort, paidFrom: draft.paidFrom, paidTo: draft.paidTo }))}>Apply</Button></div></div> }
