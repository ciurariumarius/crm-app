...
```bash
/usr/bin/curl --fail --silent --show-error --request POST --header "Authorization: Bearer <CRON_SECRET>" https://crm.pixelist.ro/api/cron/lms-daily-admin-work
```
...
5 8 * * * /usr/bin/curl --fail --silent --show-error --request POST --header "Authorization: Bearer <CRON_SECRET>" https://crm.pixelist.ro/api/cron/lms-daily-admin-work
```
...
```bash
/usr/bin/curl --fail --silent --show-error --request POST --header "Authorization: Bearer <CRON_SECRET>" "https://crm.pixelist.ro/api/cron/lms-daily-admin-work?dryRun=1"
```
