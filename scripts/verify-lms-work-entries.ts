...
async function run() {
  ...
  assert.deepEqual(Array.from(LMS_WORK_ENTRY_ORIGINS), ["MANUAL", "RECURRENCE", "CRM_TASK"])
  assert.deepEqual(Array.from(LMS_WORK_ORIGIN_FILTERS), ["all", "MANUAL", "RECURRENCE", "CRM_TASK"])
  assert.equal(normalizeLmsWorkOriginFilter(undefined), "all")
  assert.equal(normalizeLmsWorkOriginFilter("CRM_TASK"), "CRM_TASK")
  assert.equal(normalizeLmsWorkOriginFilter("invalid"), "all")
  assert.equal(normalizeLmsWorkEntryOrigin("RECURRENCE"), "RECURRENCE")
  assert.equal(normalizeLmsWorkEntryOrigin("invalid"), "MANUAL")
  ...
  assert.deepEqual(
    buildLmsWorkEntryWhere({ from: null, to: null, origin: "CRM_TASK" }),
    { origin: "CRM_TASK" }
  )
  assert.deepEqual(buildLmsWorkEntryWhere({ from: null, to: null, origin: "all" }), {})
  ...
  const addClientDialogSource = readFileSync(resolve(process.cwd(), "components/lms-work-entries/add-lms-client-dialog.tsx"), "utf8")
  ...
  assert.match(workLogSource, /disabled=\{disabled \? true : isWeekend\}/)
  assert.match(workLogSource, /if \(disabled \|\| !date \|\| isWeekend\(date\)\) return/)
  ...
  assert.match(workLogSource, /body: JSON\.stringify\(\{ ids: selectedEntryIds, origin: data\.origin \}\)/)
  ...
  assert.match(workLogSource, /colSpan=\{8\}/)
  ...
  assert.match(workLogSource, /AddLmsClientDialog/)
  assert.match(addClientDialogSource, /createLmsWorkClient/)
  assert.match(workLogSource, /Use or add/)
  assert.match(workLogSource, /onCreateRequest/)
  assert.match(addClientDialogSource, /If it already exists, it will be selected instead of duplicated\./)
  ...
  assert.match(workLogSource, /query\.set\("origin", data\.origin\)/)
  ...
  assert.match(workLogSource, /Filter work entries by source/)
  assert.match(workLogSource, /triggerLabel="Source"/)
  assert.match(workLogSource, /options=\{ORIGIN_FILTER_OPTIONS\}/)
  assert.match(workLogSource, /value=\{data\.origin\}/)
  assert.match(workLogSource, /OriginBadge origin=\{entry\.origin\}/)
  assert.match(workLogSource, /CRM: \{entry\.crmTaskName\}/)
  assert.match(workLogSource, /reopenTask\(entry\.crmTaskId\)/)
  assert.match(workLogSource, /editLocked = isCrmTaskEntry && Boolean\(entry\.exportedAt\)/)
  ...
  assert.match(workLogPageSource, /normalizeLmsWorkOriginFilter\(params\.origin\)/)
  ...
  assert.match(workLogDbSource, /buildLmsWorkEntryWhere\(\{ from, to, workDate, clientId, taskId, origin \}\)/)
  assert.match(workLogDbSource, /normalizeLmsWorkExportStatus\(args\?\.exportStatus\)/)
  assert.match(workLogDbSource, /buildLmsWorkEntryWhere\(\{ from, to, workDate, clientId, taskId, origin, exportStatus \}\)/)
  assert.match(workLogDbSource, /buildLmsWorkEntryWhere\(\{ from, to, clientId, taskId, origin, exportStatus \}\)/)
  assert.match(workLogDbSource, /buildLmsWorkEntryWhere\(\{ from, to, workDate, taskId, origin, exportStatus \}\)/)
  assert.match(workLogDbSource, /buildLmsWorkEntryWhere\(\{ from, to, workDate, clientId, origin, exportStatus \}\)/)
  ...
  assert.match(workLogDbSource, /origin: \{ not: "CRM_TASK" \}/)
  assert.match(workLogDbSource, /crmTaskNameSnapshot: true/)
  ...
  assert.match(workEntryActionsSource, /origin: "MANUAL"/)
  assert.match(workEntryActionsSource, /existing\.origin === "CRM_TASK" && existing\.exportedAt/)
  assert.match(workEntryActionsSource, /existing\.origin === "CRM_TASK"/)
  assert.match(workEntryActionsSource, /if \(existing\.crmTaskId\)/)
  assert.match(workEntryActionsSource, /where: \{ id: existing\.id, exportedAt: null \}/)
  assert.match(workEntryActionsSource, /isLmsWorkWeekday\(validated\.workDate\)/)
  assert.match(workEntryActionsSource, /tx\.task\.update\([\s\S]*lmsAllocationId: validated\.lmsAllocationId[\s\S]*lmsTaskTypeId: validated\.taskTypeId/)
  assert.match(workEntryActionsSource, /LMS_CRM_TASK_ENTRY_MANAGED_BY_TASK/)
  ...
  assert.match(workEntryExportRouteSource, /buildLmsWorkEntryWhere\(\{ from, to, workDate, clientId, taskId, origin \}\)/)
  ...
  assert.match(workEntryExportRouteSource, /searchParams\.get\("origin"\)/)
  assert.match(workEntryExportRouteSource, /normalizeLmsWorkOriginFilter/)
  ...
  assert.match(workEntryExportRouteSource, /updatedAt: true/)
  assert.match(workEntryExportRouteSource, /updatedAt: entry\.updatedAt/)
  ...
  assert.match(workEntryExportRouteSource, /where: \{ id: \{ in: ids \}, \.\.\.originWhere \}/)
  ...
  assert.match(workEntryExportRouteSource, /auditDetails: `mode=selected; origin=\$\{origin\}`/)
  assert.match(workEntryExportRouteSource, /X-Exported-Entry-Count/)
  assert.doesNotMatch(workEntryExportRouteSource, /crmTaskNameSnapshot/)
  ...
  assert.match(prismaSchemaSource, /origin\s+String\s+@default\("MANUAL"\)/)
  assert.match(prismaSchemaSource, /crmTaskId\s+String\?\s+@unique\s+@map\("crm_task_id"\)/)
  ...
  assert.match(dailyAdminAutomationSource, /origin: "MANUAL"/)
  assert.match(dailyAdminAutomationSource, /data: \{ sourceKey, origin: "RECURRENCE" \}/)
  ...
  assert.match(recurringWorkSource, /AddLmsClientDialog/)
